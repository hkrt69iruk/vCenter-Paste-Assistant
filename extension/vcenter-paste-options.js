(function () {
  ...
  function tr(text) { return window.VCenterPasteI18n ? window.VCenterPasteI18n.t(text) : text; }
  ...
  const POPUP_DEFAULT_TAB_KEY = 'vcpa_popup_default_tab';
  ...
  function loadSettings() {
    Promise.all([
      ...
      if (settingsLanguage) settingsLanguage.value = results[9] === 'en' ? 'en' : 'zh-CN';
    });
  }

  if (settingsLanguage) {
    settingsLanguage.addEventListener('change', function () {
      const language = settingsLanguage.value === 'en' ? 'en' : 'zh-CN';
      storageSet(LANGUAGE_KEY, language).then(function () {
        if (window.VCenterPasteI18n) window.VCenterPasteI18n.setLanguage(language);
      });
    });
  }
  ...
  if (settingsKeystrokeDelay) {
    settingsKeystrokeDelay.addEventListener('change', function () {
      const parsed = Number(settingsKeystrokeDelay.value);
      const val = Math.max(0, Math.min(500, Number.isFinite(parsed) ? parsed : DEFAULT_KEYSTROKE_DELAY_MS));
  ...
  if (settingsFirstCharDelay) {
    settingsFirstCharDelay.addEventListener('change', function () {
      const parsed = Number(settingsFirstCharDelay.value);
      const val = Math.max(0, Math.min(1000, Number.isFinite(parsed) ? parsed : DEFAULT_FIRST_CHAR_DELAY_MS));
  ...
  if (settingsEnterDelay) {
    settingsEnterDelay.addEventListener('change', function () {
      const parsed = Number(settingsEnterDelay.value);
      const val = Math.max(0, Math.min(300, Number.isFinite(parsed) ? parsed : DEFAULT_ENTER_DELAY_MS));
    ...
    fileInput.addEventListener('change', function () {
      ...
      reader.onload = function () {
        ...
        try {
          ...
          setStatus(tr('Import failed: invalid JSON.'), true);
          return;
        }
        if (!list || list.length === 0) {
          setStatus(tr('Import failed: no snippets in file.'), true);
          return;
        }

        const normalized = list.map(normalizeSnippet).filter(Boolean);
        if (normalized.length === 0) {
          setStatus(tr('Import failed: no valid snippets.'), true);
          return;
        }
        ...
        getSnippets().then(function (existing) {
          ...
          (function () {
            ...
            setSnippets(capped).then(function () {
              setStatus(tr('Imported {count} snippet(s). Total: {total}.').replace('{count}', normalized.length).replace('{total}', capped.length));
            });
          })();
        });
      };
      reader.onerror = function () {
        setStatus(tr('Import failed: could not read file.'), true);
      };
