(function() {
  ...
  const PASTE_DRAFT_KEY = 'vcpa_paste_draft';
  ...
  const popupPasteText = document.getElementById('vcpa-textarea');
  ...
  const settingsUrl = api ? api.runtime.getURL('vcenter-paste-options.html') : '#';
  if (cpSettingsBtn) {
  ...
  function setPasteHint(msg, isError) {
    if (popupPasteHint) {
      ...
      popupPasteHint.style.color = isError ? 'var(--vcpa-accent)' : 'var(--vcpa-textMuted)';
    }
  }
  ...
  function renderSnippetList() {
    ...
    getSnippets().then(function(snippets) {
      ...
      if (filtered.length === 0) {
        ...
        empty.textContent = tr(q ? 'No snippets match.' : 'No snippets yet. Add one below or open Settings to import.');
      ...
      filtered.forEach(function(s, idx) {
        ...
        runBtn.type = 'button';
        runBtn.className = 'vcpa-snip-send';
        runBtn.textContent = tr('Send');
        runBtn.addEventListener('click', function() {
          if (!s.text) return;
          sendToContentScript({ action: 'sendText', text: s.text }).then(function(r) {
            if (r.ok) setPasteHint(tr('Paste started.'));
            else setPasteHint(tr(r.error || 'Could not send.'), true);
          });
        });
        const editBtn = document.createElement('button');
        editBtn.type = 'button';
        editBtn.className = 'vcpa-snip-edit';
        ...
        editBtn.title = tr('Edit');
        editBtn.addEventListener('click', function() {
          showSnippetForm(s);
        });
        const delBtn = document.createElement('button');
        delBtn.type = 'button';
        delBtn.className = 'vcpa-snip-del';
        delBtn.textContent = '\u2715';
        ...
        if (canReorder) {
          ...
          dragHandle.addEventListener('dragend', function() {
            popupSnippetList.querySelectorAll('.vcpa-snip-row').forEach(function(r) {
  ...
  if (popupSaveAsSnippetBtn && popupPasteText) {
    popupSaveAsSnippetBtn.addEventListener('click', function() {
      ...
      if (!text) {
        setPasteHint(tr('Type or paste text above first, then click Save as snippet.'), true);
        return;
      }
      setTab('snippets'); // form lives in Snippets view – switch so it’s visible
      showSnippetForm({ id: null, name: '', text: text });
      setPasteHint(tr('Name the snippet and click Save.'));
    });
  }
  ...
  if (popupSendBtn && popupPasteText) {
    popupSendBtn.addEventListener('click', function() {
      const text = (popupPasteText.value || '').trim();
      if (!text) {
        setPasteHint(tr('Enter or paste text above, then click Send.'), true);
        return;
      }
      setPasteHint(tr('Pasting…'));
      sendToContentScript({ action: 'sendText', text: text }).then(function(r) {
        if (r.ok) {
          setPasteHint(tr('Paste started.'));
        } else {
