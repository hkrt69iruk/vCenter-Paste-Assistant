(function () {
  ...
  function updateStats(snippets) {
    ...
    if (statRecent) {
      ...
      if (latest) {
        ...
        statRecent.textContent = days  > 0 ? days  + t('d')
                               : hrs   > 0 ? hrs   + t('h')
                               : mins  > 0 ? mins  + t('m')
                               : t('now');
      } else {
  ...
  function renderList(snippets) {
    ...
    if (!snippets.length) {
      ...
      empty.textContent = t('No snippets yet — import a backup or add snippets from the extension popup.');
  ...
  function buildRow(snippet) {
    ...
    nameEl.textContent = snippet.name || t('(unnamed)');
    ...
    nameField.innerHTML = '<label class="edit-label">' + t('Name') + '</label>';
    ...
    textField.innerHTML = '<label class="edit-label">' + t('Text') + '</label>';
    ...
    saveBtn.addEventListener('click', function () {
      ...
      loadSnippets(function (current) {
        var updated = current.map(function (s) {
          ...
          return { id: s.id, name: newName || t('Snippet'), text: newText, updatedAt: Date.now() };
        });
        saveSnippets(updated, function () {
          snippet.name = newName || t('Snippet');
  ...
  exportBtn.addEventListener('click', function () {
    loadSnippets(function (snippets) {
      ...
      a.href     = url;
      a.download = 'vcenter-paste-assistant-snippets-' + new Date().toISOString().slice(0, 10) + '.json';
  ...
  if (window.VCenterPasteI18n) {
    window.VCenterPasteI18n.onReady(function () { loadSnippets(renderList); });
  } else {
